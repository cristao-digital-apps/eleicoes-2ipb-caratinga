...
export async function saveEndpointsCache(cache,sheetId,endpoints){const normalized=validateEndpoints(endpoints),fingerprint=await endpointFingerprint(normalized),value={sheetId,endpoints:normalized,fingerprint};cache.put('endpointsCache',value);return value;}
export async function loadEndpointsCache(cache,sheetId){const value=cache.json('endpointsCache');if(!value||value.sheetId!==sheetId||!Array.isArray(value.endpoints))return null;try{const endpoints=validateEndpoints(value.endpoints),fingerprint=await endpointFingerprint(endpoints);return fingerprint===value.fingerprint?{sheetId,endpoints,fingerprint}:null;}catch{return null;}}
export async function saveSheetCache(cache,sheetId,ballot,now=Date.now()){const payload=JSON.stringify(ballot),value={sheetId,fetchedAt:now,hash:await sha(payload),ballot};cache.put('sheetCache',value);return ballot;}
export async function loadSheetCache(cache,sheetId,{maxAgeMs=SHEET_CACHE_MAX_AGE_MS,now=Date.now()}={}){const value=cache.json('sheetCache');if(!value||value.sheetId!==sheetId||!Number.isFinite(value.fetchedAt)||now-value.fetchedAt<0||now-value.fetchedAt>maxAgeMs||!value.ballot)return null;try{return await sha(JSON.stringify(value.ballot))===value.hash?value.ballot:null;}catch{return null;}}
export async function fetchSheetCached(sheetId,cache,{force=false,maxAgeMs=SHEET_CACHE_MAX_AGE_MS}={}){if(!force){const cached=await loadSheetCache(cache,sheetId,{maxAgeMs});if(cached)return cached;}return saveSheetCache(cache,sheetId,await fetchSheet(sheetId));}
...
export async function readPages(fetchPage,{after=0,limit=200}={}){const records=[],hashes=new Map();let cursor=after,more=true;while(more){const page=await fetchPage(cursor,limit);if(!page||!Array.isArray(page.records)||!Number.isSafeInteger(page.nextAfter)||typeof page.hasMore!=='boolean')throw Error('Página inválida no endpoint.');for(const record of page.records){if(!Number.isSafeInteger(record.cursor)||record.cursor!==cursor+1||typeof record.hash!=='string'||!record.data||await sha(JSON.stringify(record.data))!==record.hash||hashes.has(record.cursor))throw Error('Falha de integridade no endpoint.');hashes.set(record.cursor,record.hash);cursor=record.cursor;records.push({...record.data,_cursor:record.cursor,_hash:record.hash,_receivedAt:record.receivedAt});}if(page.nextAfter!==cursor||(page.hasMore&&!page.records.length))throw Error('Paginação incoerente no endpoint.');more=page.hasMore;}return {records,lastCursor:cursor};}
export async function adminPaged(endpoint,action,sessionId,capability,privateJwk,options={}){return readPages((after,limit)=>adminRequest(endpoint,action,sessionId,capability,privateJwk,{...options,after,limit}));}
export async function publicPaged(endpoint,action,data,options={}){return readPages((after,limit)=>publicApi(endpoint,action,{...data,...options,after,limit}));}
